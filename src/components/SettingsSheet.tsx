...
export function SettingsSheet({ settings, dispatch, onClose }: SettingsSheetProps) {
  ...
  return (
    <Sheet title="Settings" onClose={onClose}>
      ...
      <label className="setting-toggle"><span><strong>Sound effects</strong><small>Play combat, magic, healing, and equipment cues.</small></span><input aria-label="Sound effects" type="checkbox" checked={settings.sound} onChange={(event) => update({ sound: event.target.checked })} /></label>
