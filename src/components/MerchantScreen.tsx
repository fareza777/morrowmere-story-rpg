...
export function MerchantScreen({ view, onBuy, onSell, onClose }: MerchantScreenProps) {
  ...
  return (
    <main className="merchant-screen screen-page">
      ...
      {view.illustrationId && <SceneArt illustrationId={view.illustrationId} alt={view.illustrationAlt} />}
      ...
      {tab === 'buy' ? (
        view.stock.length === 0 ? <p className="empty-state">{view.emptyStockMessage ?? `${view.name} has no stock left for this visit.`}</p> : <div className="merchant-list">{view.stock.map((item) => <article key={item.stockEntryId}><header><div><strong>{item.name}</strong><span>{item.rarityLabel} · {item.categoryLabel}</span></div><b>{item.price} gold</b></header><p>{item.description}</p><button type="button" aria-label={`Buy ${item.name} for ${item.price} gold`} disabled={!item.affordable} onClick={() => onBuy(item.stockEntryId)}>Buy for {item.price} gold</button>{!item.affordable && <small>Insufficient gold. Your current balance is {view.totalGold}.</small>}</article>)}</div>
      ) : (
        view.sellable.length === 0 ? <p className="empty-state">Nothing in your pack can be sold here.</p> : <div className="merchant-list">{view.sellable.map((item) => <article key={item.entryId ?? item.itemId}><header><div><strong>{item.name}</strong><span>Quantity {item.quantity}</span></div><b>{item.priceEach} each</b></header><p>{item.description}</p><button type="button" aria-label={`Sell ${item.name} for ${item.stackPrice} gold`} onClick={() => item.entryId && onSell(item.entryId, item.quantity)}>Sell stack for {item.stackPrice} gold</button></article>)}</div>
      )}
    </main>
  );
}
