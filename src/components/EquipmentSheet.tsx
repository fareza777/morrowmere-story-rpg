...
function slotItem(label: string, item: ItemRowViewModel | null, icon: 'weapon' | 'armor' | 'charm', canUnequip: boolean, onCommand: (command: UiInventoryCommand) => void) {
  ...
  return <article><Icon size={21} aria-hidden="true" /><span><small>{label}</small><strong>{item?.name ?? 'Empty'}</strong></span>{item && <button type="button" disabled={!canUnequip} onClick={() => onCommand({ type: 'unequip', itemId: item.itemId as ItemId })}>Unequip {item.name}</button>}</article>;
}
  ...
  return (
    <div className="equipment-page">
      ...
      {gear.length === 0 ? <p className="empty-state">No alternative equipment is in the pack.</p> : <div className="inventory-list loadout-list">{gear.map((item) => {
        ...
        return <article key={item.entryId ?? item.itemId}><header><strong>{item.name}</strong><small>{item.rarityLabel} · {item.categoryLabel}</small></header><p>{itemDelta(item, current)}</p>{item.restrictionLabel && <p className="restriction-copy">{item.restrictionLabel}</p>}<button className="loadout-action" type="button" aria-label={`${action} ${item.name}`} disabled={disabled || !item.entryId} onClick={() => item.entryId && onCommand({ type: 'equip', entryId: item.entryId, heroClass })}>{action} {item.name}</button>{context === 'combat' && <small>Equipment is locked during battle.</small>}{charmsFull && <small>Both charm slots are full.</small>}</article>;
      })}</div>}
    </div>
  );
}
