...
export function CompanionPanel({ companions, canSwitch, onSetActive }: CompanionPanelProps) {
  if (companions.length === 0) return <p className="empty-state">No companion has joined your road. Recruitment begins through story choices.</p>;
  return (
    <div className="companion-list">
      {companions.map((companion) => (
        <article key={companion.id} className={companion.active ? 'is-active' : ''}>
          ...
          <p>{companion.loyaltyDescription}</p>
          {companion.recruitmentCostLabel && companion.status === 'unknown' && <p className="recruitment-hint">Recruitment clue: {companion.recruitmentCostLabel}</p>}
          ...
          {companion.personalQuests.length > 0 && <div className="quest-stages"><h3>Personal quest</h3><ol>{companion.personalQuests.map((quest) => <li key={quest.id} className={quest.completed ? 'is-complete' : ''}>{quest.completed && <Check size={15} aria-label="Completed" />}<div><strong>{quest.title}</strong><small>Stage {quest.stage} · {quest.completed ? 'Complete' : 'Open'}</small><p>{quest.summary}</p></div></li>)}</ol></div>}
      ...
      {canSwitch && companions.some((companion) => companion.active) && <button className="button button-secondary" type="button" onClick={() => onSetActive(null)}>Travel Alone</button>}
    </div>
  );
}
