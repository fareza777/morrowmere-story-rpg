...
import { callbackScene, eligibleScenes } from './eligibility';
import { nextTension, nextThreat, pacingCandidates, routeWeight, scenePacing } from './pacing';
import type { DirectorContext, DirectorReason, DirectorState, DirectorStep } from './types';

const THREAT_ENCOUNTER_THRESHOLD = 6;
const RECENT_FAMILY_WINDOW = 3;

function weightedPick(events: readonly ChronicleEvent[], random: number, context: DirectorContext): ChronicleEvent {
...
function selectPacedEvent(events: readonly ChronicleEvent[], state: DirectorState, context: DirectorContext, random: number): ChronicleEvent {
...
export function selectNextScene(state: DirectorState, context: DirectorContext, content: ContentIndex): DirectorStep {
  ...
  if (!event) {
    throw new Error(`No eligible Chronicle scene is available for ${context.position.chapterId} slot ${context.position.slot}.`);
  }
  return { sceneId: event.id, event, reason, state: nextState(state, event, rng.state) };
}
