...
import type { CreateCampaignOptions, GameStateV2, ProfileState } from './types';
...
export function createCampaign(options: CreateCampaignOptions, content: ContentIndex): GameStateV2 {
  ...
  const campaign = {
    ...
    bankedGold: 12,
    flags: [], evidence: [], factions: {}, companions: createCompanionRoster(content),
    attemptCounters: {}, routeSeedNonce: 0, transitionCounter: 0,
  } as const;
  const payload = campaignPayload(campaign);
  return {
    schemaVersion: 2, profile: DEFAULT_PROFILE, campaign, expedition: null,
    checkpoints: { chapter: { campaign: payload, enteredAt: options.updatedAt }, camp: { campaign: payload, campSceneId: null, savedAt: options.updatedAt } },
