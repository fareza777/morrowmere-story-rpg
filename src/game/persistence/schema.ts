...
export interface MerchantVisitDto { readonly merchantId: string; readonly restockKey: string; readonly stock: readonly { readonly id: string; readonly itemId: string }[]; }
...
function validCombat(value: unknown): boolean {
  ...
  if (!enemyIds.has(primaryEnemyId) || typeof value.enemyIntent !== 'string' || !combatIntents.has(value.enemyIntent) || !Array.isArray(enemyIntents) || !enemyIntents.every(validCombatIntent) || !enemyIntents.every((intent) => enemyIds.has(intent.enemyId))) return false;
...
function validMerchantVisit(value: unknown): boolean { return exact(value, ['merchantId', 'restockKey', 'stock']) && nonEmptyString(value.merchantId) && nonEmptyString(value.restockKey) && Array.isArray(value.stock) && value.stock.every((entry) => exact(entry, ['id', 'itemId']) && nonEmptyString(entry.id) && nonEmptyString(entry.itemId)); }
