...
export interface ExpeditionDto { readonly routeProfile: 'kings-road' | 'old-forest' | 'ruined-pass'; readonly routeSeed: number; readonly director: DirectorDto; readonly position: { readonly chapterId: string; readonly slot: number }; readonly currentSceneId: string | null; readonly sceneResolution: { readonly eventId: string; readonly choiceId: string | null } | null; readonly heroVitals: { readonly health: number; readonly resource: number }; readonly currentCombat: { readonly encounterId: string; readonly combat: CombatDto | null } | null; readonly pendingReward: { readonly rewardId: string; readonly encounterId: string; readonly itemChoices: readonly string[]; readonly baseGold: number; readonly grantedXp: number; readonly adEligible: boolean } | null; readonly unbankedGold: number; readonly unbankedLoot: readonly string[]; readonly temporaryBoons: readonly string[]; readonly merchantVisits: readonly MerchantVisitDto[]; }
...
function validExpedition(value: unknown): boolean {
  ...
  const pendingReward = value.pendingReward === null || (exact(value.pendingReward, ['rewardId', 'encounterId', 'itemChoices', 'baseGold', 'grantedXp', 'adEligible']) && nonEmptyString(value.pendingReward.rewardId) && nonEmptyString(value.pendingReward.encounterId) && uniqueStrings(value.pendingReward.itemChoices) && number(value.pendingReward.baseGold, 0, true) && number(value.pendingReward.grantedXp, 0, true) && typeof value.pendingReward.adEligible === 'boolean');
  return typeof value.routeProfile === 'string' && routeProfiles.has(value.routeProfile) && number(value.routeSeed, 0, true) && validDirector(value.director) && validPosition(value.position) && idOrNull(value.currentSceneId) && sceneResolution && heroVitals && (value.currentCombat === null || (exact(value.currentCombat, ['encounterId', 'combat']) && nonEmptyString(value.currentCombat.encounterId) && (value.currentCombat.combat === null || validCombat(value.currentCombat.combat)))) && pendingReward && number(value.unbankedGold, 0, true) && stringArray(value.unbankedLoot) && stringArray(value.temporaryBoons) && Array.isArray(value.merchantVisits) && value.merchantVisits.every(validMerchantVisit);
}
