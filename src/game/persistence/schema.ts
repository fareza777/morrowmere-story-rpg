...
export interface PlayerModifierDto { readonly attackBonus: number; readonly armor: number; readonly ward: number; readonly maxHealth: number; readonly maxFocus: number; readonly strength: number; readonly cunning: number; readonly will: number; readonly attackAccuracy?: number; readonly criticalChance?: number; }
...
const flowScreens = new Set(['camp', 'travel', 'story', 'combat', 'reward', 'merchant', 'defeat', 'ending']);
...
function validPlayerModifiers(value: unknown): boolean {
  const required = ['attackBonus', 'armor', 'ward', 'maxHealth', 'maxFocus', 'strength', 'cunning', 'will'];
  const optional = ['attackAccuracy', 'criticalChance'];
  if (!record(value) || required.some((key) => !Object.hasOwn(value, key)) || Object.keys(value).some((key) => !required.includes(key) && !optional.includes(key)) || !Object.values(value).every((entry) => number(entry))) return false;
  const attackAccuracy = value.attackAccuracy;
  const criticalChance = value.criticalChance;
  return (attackAccuracy === undefined || (number(attackAccuracy, 0) && attackAccuracy <= 100))
    && (criticalChance === undefined || (number(criticalChance, 0) && criticalChance <= 100));
}
