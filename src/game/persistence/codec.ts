...
export function decodeSaveStateWithDiagnostics(value: unknown, content: ContentIndex): DecodedSaveState | null {
  ...
  const expedition = sanitized.expedition === null ? null : decodeExpedition(sanitized.expedition, campaign, content);
  if (sanitized.expedition !== null && !expedition) return null;
  ...
  const diagnostics = [...(migrated?.diagnostics ?? []), ...(normalized.normalized ? ['Restored dialogue progress at the first beat for this v3 save.'] : []), ...(removed > 0 && !migrated ? [`Removed ${removed} unavailable authored scene ${removed === 1 ? 'entry' : 'entries'} while recovering the save.`] : [])];
