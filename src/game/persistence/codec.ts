...
const expeditionRuntimeKeys = [...expeditionRuntimeLegacyKeys, 'lastTravelAction'] as const;
...
function validRuntimeShell(state: GameStateV2): boolean {
  if (!exact(state, rootKeys) || state.schemaVersion !== 3 || !exact(state.checkpoints, ['chapter', 'camp']) || !exact(state.checkpoints.chapter, ['campaign', 'enteredAt']) || typeof state.checkpoints.chapter.enteredAt !== 'string' || !exact(state.flow, ['screen', 'overlay', 'merchant']) || typeof state.updatedAt !== 'string') return false;
