...
import { migrateSaveV2 } from './migrate';
import {
  isProfileState,
  isJsonCompatible,
  isPlainRecord,
  isSceneResolutionDto,
  isSaveStateV2Dto,
  isSaveStateDto,
  isSaveStateV3Dto,
  validDungeonRun,
...
const expeditionRuntimeLegacyKeys = ['routeProfile', 'routeSeed', 'director', 'position', 'currentSceneId', 'dialogueBeatIndex', 'sceneResolution', 'authoredSceneQueue', 'sceneVisitCounts', 'checkedAttempts', 'heroVitals', 'currentCombat', 'pendingReward', 'unbankedGold', 'unbankedLoot', 'temporaryBoons', 'merchantVisits'] as const;
const expeditionRuntimeV3Keys = [...expeditionRuntimeLegacyKeys, 'lastTravelAction'] as const;
const expeditionRuntimeKeys = [...expeditionRuntimeV3Keys, 'dungeonRun', 'pendingRouteJunctionId'] as const;
const travelActionIds = new Set(['scout', 'press-on', 'make-camp', 'companion']);
const knownRoadBoonIds = new Set([
  'road:scouted',
  'road:pressed',
  'road:rested',
  'road:guarded',
...
function validExpeditionContent(value: ExpeditionDto, campaign: CampaignDto, flow: SaveStateDto['flow'], content: ContentIndex): boolean {
  if (value.pendingRouteJunctionId !== null) {
    const junction = content.routeJunctions?.get(value.pendingRouteJunctionId);
    if (!junction || junction.chapterId !== value.position.chapterId) return false;
  }
  if (value.dungeonRun !== null) {
    const dungeon = content.dungeons?.get(value.dungeonRun.dungeonId);
    if (!dungeon || dungeon.chapterId !== value.position.chapterId
      || !dungeon.nodes.some((node) => node.id === value.dungeonRun!.currentNodeId)
      || !value.dungeonRun.visitedNodeIds.every((id) => dungeon.nodes.some((node) => node.id === id))
      || !value.dungeonRun.resolvedNodeIds.every((id) => dungeon.nodes.some((node) => node.id === id))) return false;
  }
...
function encodeExpedition(value: ExpeditionState, campaign: CampaignState, campaignDto: CampaignDto, content: ContentIndex): ExpeditionDto | null {
  if ((!exact(value, expeditionRuntimeKeys) && !exact(value, expeditionRuntimeV3Keys) && !exact(value, expeditionRuntimeLegacyKeys)) || !['kings-road', 'old-forest', 'ruined-pass'].includes(value.routeProfile) || !finiteInteger(value.routeSeed) || value.routeSeed < 0 || !exact(value.position, ['chapterId', 'slot']) || typeof value.position.chapterId !== 'string' || !finiteInteger(value.position.slot) || value.position.slot < 0 || (value.currentSceneId !== null && !id(value.currentSceneId)) || !finiteInteger(value.dialogueBeatIndex) || value.dialogueBeatIndex < 0 || (value.lastTravelAction !== undefined && value.lastTravelAction !== null && !travelActionIds.has(value.lastTravelAction)) || (value.sceneResolution !== null && !isSceneResolutionDto(value.sceneResolution)) || !Array.isArray(value.authoredSceneQueue) || !value.authoredSceneQueue.every((entry) => (exact(entry, ['sceneId', 'sourceSceneId', 'requirementMode']) || (exact(entry, ['sceneId', 'sourceSceneId', 'requirementMode', 'reason']) && typeof entry.reason === 'string' && id(entry.reason))) && id(entry.sceneId) && id(entry.sourceSceneId) && typeof entry.requirementMode === 'string' && ['required', 'optional'].includes(entry.requirementMode)) || !record(value.sceneVisitCounts) || !Object.entries(value.sceneVisitCounts).every(([eventId, count]) => id(eventId) && finiteInteger(count) && count >= 1) || !Array.isArray(value.checkedAttempts) || !value.checkedAttempts.every((attempt) => exact(attempt, ['eventId', 'choiceId', 'visitOrdinal', 'chance', 'roll', 'resultKind']) && id(attempt.eventId) && id(attempt.choiceId) && finiteInteger(attempt.visitOrdinal) && attempt.visitOrdinal >= 1 && finiteInteger(attempt.chance) && attempt.chance >= 0 && attempt.chance <= 100 && finiteInteger(attempt.roll) && attempt.roll >= 1 && attempt.roll <= 100 && ['critical-success', 'success', 'failure', 'critical-failure'].includes(attempt.resultKind as string)) || !exact(value.heroVitals, ['health', 'resource']) || !finite(value.heroVitals.health) || value.heroVitals.health < 0 || !finite(value.heroVitals.resource) || value.heroVitals.resource < 0 || (value.pendingReward !== null && (!exact(value.pendingReward, ['rewardId', 'rewardOfferId', 'encounterId', 'itemChoices', 'baseGold', 'grantedXp', 'adEligible', 'rewardedGoldSettlement']) || !id(value.pendingReward.rewardId) || !id(value.pendingReward.rewardOfferId) || !id(value.pendingReward.encounterId) || !strings(value.pendingReward.itemChoices) || !finiteInteger(value.pendingReward.baseGold) || value.pendingReward.baseGold < 0 || !finiteInteger(value.pendingReward.grantedXp) || value.pendingReward.grantedXp < 0 || typeof value.pendingReward.adEligible !== 'boolean' || !['available', 'claimed', 'ineligible'].includes(value.pendingReward.rewardedGoldSettlement))) || !finiteInteger(value.unbankedGold) || value.unbankedGold < 0 || !strings(value.unbankedLoot) || !strings(value.temporaryBoons) || !Array.isArray(value.merchantVisits)) return null;
  if (value.dungeonRun !== null && value.dungeonRun !== undefined && !validDungeonRun(value.dungeonRun)) return null;
  ...
  const sceneResolution = value.sceneResolution === null ? null : { ...value.sceneResolution, effectSummary: [...value.sceneResolution.effectSummary] } as SceneResolutionDto;
  return { dungeonRun: value.dungeonRun === null || value.dungeonRun === undefined ? null : { ...value.dungeonRun, visitedNodeIds: [...value.dungeonRun.visitedNodeIds], resolvedNodeIds: [...value.dungeonRun.resolvedNodeIds] }, pendingRouteJunctionId: value.pendingRouteJunctionId ?? null, routeProfile: value.routeProfile, routeSeed: value.routeSeed, director, position: { chapterId: value.position.chapterId, slot: value.position.slot }, currentSceneId: value.currentSceneId, dialogueBeatIndex: value.dialogueBeatIndex, sceneResolution, authoredSceneQueue: value.authoredSceneQueue.map((entry) => ({ ...entry })), sceneVisitCounts: { ...value.sceneVisitCounts }, checkedAttempts: value.checkedAttempts.map((attempt) => ({ ...attempt })), heroVitals: { ...value.heroVitals }, currentCombat: currentCombat as ExpeditionDto['currentCombat'], pendingReward: value.pendingReward ? { ...value.pendingReward, itemChoices: [...value.pendingReward.itemChoices] } : null, unbankedGold: value.unbankedGold, unbankedLoot: [...value.unbankedLoot], temporaryBoons: [...value.temporaryBoons], lastTravelAction: value.lastTravelAction ?? null, merchantVisits: merchantVisits as readonly ExpeditionDto['merchantVisits'][number][] };
}

function validRuntimeShell(state: GameStateV2): boolean {
  if (!exact(state, rootKeys) || state.schemaVersion !== 4 || !exact(state.checkpoints, ['chapter', 'camp']) || !exact(state.checkpoints.chapter, ['campaign', 'enteredAt']) || typeof state.checkpoints.chapter.enteredAt !== 'string' || !exact(state.flow, ['screen', 'overlay', 'merchant']) || typeof state.updatedAt !== 'string') return false;
...
function decodeSceneResolution(value: SceneResolutionDto | null): ExpeditionState['sceneResolution'] {
  if (value === null) return null;
  if (value.resultKind === 'direct') return {
    eventId: value.eventId as never, choiceId: value.choiceId as never, resultKind: 'direct', chance: null, roll: null,
    outcome: value.outcome, effectSummary: [...value.effectSummary], nextSceneId: value.nextSceneId as never, continueLabel: value.continueLabel,
  };
  return {
    eventId: value.eventId as never,
    choiceId: value.choiceId as never,
    resultKind: value.resultKind,
    chance: value.chance,
    roll: value.roll,
    outcome: value.outcome,
    effectSummary: [...value.effectSummary],
    nextSceneId: value.nextSceneId as never,
    continueLabel: value.continueLabel,
  };
}
interface DecodedExpedition {
  readonly expedition: ExpeditionState;
  readonly dialogueRecovered: boolean;
  readonly unknownRoadBoonsRemoved: number;
}

function sanitizeRoadBoons(boons: readonly string[]): { readonly boons: readonly string[]; readonly unknownRemoved: number } {
  const known = boons.filter((boon) => !boon.startsWith('road:') || knownRoadBoonIds.has(boon));
  return { boons: known, unknownRemoved: boons.length - known.length };
}

function decodeExpedition(value: ExpeditionDto, campaign: CampaignState, content: ContentIndex): DecodedExpedition | null {
  const encounter = value.currentCombat === null ? null : content.encounters.get(value.currentCombat.encounterId as never);
...
function normalizeEmptyStoryV3(value: unknown): { readonly candidate: unknown; readonly normalized: boolean } {
  if (!record(value) || (value.schemaVersion !== 3 && value.schemaVersion !== 4) || !record(value.flow) || value.flow.screen !== 'story' || value.flow.merchant !== null || !record(value.expedition) || value.expedition.currentSceneId !== null || value.expedition.sceneResolution !== null || value.expedition.currentCombat !== null || value.expedition.pendingReward !== null) return { candidate: value, normalized: false };
  return { candidate: { ...value, flow: { ...value.flow, screen: 'travel' } }, normalized: true };
}

function upgradeV3(value: unknown, content: ContentIndex): { readonly candidate: unknown; readonly upgraded: boolean } {
  if (!isSaveStateV3Dto(value)) return { candidate: value, upgraded: false };
  const expedition = value.expedition;
  if (expedition === null) return { candidate: { ...value, schemaVersion: 4 }, upgraded: true };
  const receipt = value.flow.screen === 'travel' ? expedition.sceneResolution : null;
  const junction = receipt === null ? null : [...(content.routeJunctions?.values() ?? [])]
    .filter((entry) => entry.chapterId === expedition.position.chapterId
      && entry.position.chapterId === expedition.position.chapterId
      && entry.position.slot <= expedition.position.slot
      && entry.afterEventId === receipt.eventId)
    .sort((left, right) => right.position.slot - left.position.slot)[0] ?? null;
  return { candidate: { ...value, schemaVersion: 4, expedition: {
    ...expedition, dungeonRun: null, pendingRouteJunctionId: junction?.id ?? null,
  } }, upgraded: true };
}
...
export function decodeSaveStateWithDiagnostics(value: unknown, content: ContentIndex): DecodedSaveState | null {
  ...
  const emptyStory = normalizeEmptyStoryV3(preDialogue.candidate);
  const upgraded = upgradeV3(emptyStory.candidate, content);
  const candidate = upgraded.candidate;
  ...
  const sanitized: SaveStateDto = candidate.expedition === null || removed === 0
    ? candidate
    : { ...candidate, expedition: { ...candidate.expedition, authoredSceneQueue: validQueue } };
  if (!isContentBackedSaveState(sanitized, content)) return null;
  const campaign = decodeCampaign(sanitized.campaign);
  const decodedExpedition = sanitized.expedition === null ? null : decodeExpedition(sanitized.expedition, campaign, content);
  if (sanitized.expedition !== null && !decodedExpedition) return null;
  const expedition = decodedExpedition?.expedition ?? null;
  const adPacing = sanitized.adPacing ?? initialAdPacingState();
  const state: GameStateV2 = { schemaVersion: 4, profile: decodeProfile(sanitized.profile), campaign, expedition, adPacing: { ...adPacing, claimedRewardOfferIds: [...adPacing.claimedRewardOfferIds] }, checkpoints: { chapter: { campaign: decodeCampaignCheckpoint(sanitized.checkpoints.chapter.campaign), enteredAt: sanitized.checkpoints.chapter.enteredAt }, camp: sanitized.checkpoints.camp === null ? null : { campaign: decodeCampaignCheckpoint(sanitized.checkpoints.camp.campaign), campSceneId: sanitized.checkpoints.camp.campSceneId as never, savedAt: sanitized.checkpoints.camp.savedAt } }, flow: { screen: sanitized.flow.screen, overlay: sanitized.flow.overlay, merchant: sanitized.flow.merchant === null ? null : { merchantId: sanitized.flow.merchant.merchantId as never, restockKey: sanitized.flow.merchant.restockKey, returnScreen: sanitized.flow.merchant.returnScreen } }, updatedAt: sanitized.updatedAt };
  const diagnostics = [...(migrated?.diagnostics ?? []), ...(preDialogue.normalized ? ['Restored dialogue progress at the first beat for this v3 save.'] : []), ...(emptyStory.normalized ? ['Restored an empty story save to travel.'] : []), ...(decodedExpedition?.dialogueRecovered ? ['Adjusted dialogue progress to the current scene.'] : []), ...(decodedExpedition && decodedExpedition.unknownRoadBoonsRemoved > 0 ? [`Removed ${decodedExpedition.unknownRoadBoonsRemoved} unknown road boon ${decodedExpedition.unknownRoadBoonsRemoved === 1 ? 'entry' : 'entries'} while recovering the save.`] : []), ...(removed > 0 && !migrated ? [`Removed ${removed} unavailable authored scene ${removed === 1 ? 'entry' : 'entries'} while recovering the save.`] : [])];
  return { state, diagnostics, migrated: Boolean(migrated || upgraded.upgraded) };
}

export function decodeSaveState(value: unknown, content: ContentIndex): GameStateV2 | null {
  return decodeSaveStateWithDiagnostics(value, content)?.state ?? null;
}
