...
function encodeExpedition(value: ExpeditionState, campaign: CampaignState, campaignDto: CampaignDto, content: ContentIndex): ExpeditionDto | null {
  ...
  const sceneResolution = value.sceneResolution === null ? null : value.sceneResolution as SceneResolutionDto;
  return { routeProfile: value.routeProfile, routeSeed: value.routeSeed, director, position: { chapterId: value.position.chapterId, slot: value.position.slot }, currentSceneId: value.currentSceneId, sceneResolution, authoredSceneQueue: value.authoredSceneQueue.map((entry) => ({ ...entry })), heroVitals: { ...value.heroVitals }, currentCombat: currentCombat as ExpeditionDto['currentCombat'], pendingReward: value.pendingReward ? { ...value.pendingReward, itemChoices: [...value.pendingReward.itemChoices] } : null, unbankedGold: value.unbankedGold, unbankedLoot: [...value.unbankedLoot], temporaryBoons: [...value.temporaryBoons], merchantVisits: merchantVisits as readonly ExpeditionDto['merchantVisits'][number][] };
}
...
function decodeSceneResolution(value: SceneResolutionDto | null): ExpeditionState['sceneResolution'] {
  ...
  if (!('resultKind' in value)) return { eventId: value.eventId as never, choiceId: value.choiceId as never };
  return {
