...
const ENCOUNTER_SPECS: readonly EncounterSpec[] = [
  ...
  { id: 'enc-ch05-underworks-lift-watch-a', enemyIds: ['iron-deserter-06', 'vault-gargoyle-06'], threatBudget: 12, compatibilityTags: ['frontline', 'ranged', 'construct'], counterplay: 'Break the lift guard’s stance, then stagger the fixed shooter before it can call the next ore cage down.' , reward: { xp: 40, gold: 18, itemChoices: [] } },
  { id: 'enc-ch05-underworks-lift-watch-b', enemyIds: ['black-banner-06', 'goblin-torchling-06'], threatBudget: 12, compatibilityTags: ['mobile', 'ranged', 'false-flag', 'fire'], counterplay: 'Guard the first cinder throw and mark the reaver before it leaves the chain platform exposed.', reward: { xp: 42, gold: 16, itemChoices: [] } },
  { id: 'enc-ch05-smelter-rail-patrol-a', enemyIds: ['iron-deserter-07', 'black-banner-07'], threatBudget: 14, compatibilityTags: ['frontline', 'mobile', 'false-flag'], counterplay: 'Draw the shield away from the narrow rail, then punish the reaver after its committed hook pass.', reward: { xp: 46, gold: 21, itemChoices: [] } },
  { id: 'enc-ch05-smelter-rail-patrol-b', enemyIds: ['vault-gargoyle-07', 'ash-magus-07'], threatBudget: 15, compatibilityTags: ['construct', 'ranged', 'support', 'fire'], counterplay: 'Use the bulkhead against the magus’s hex and stagger the gargoyle before its second aimed shot.', reward: { xp: 44, gold: 19, itemChoices: [] } },
  { id: 'enc-ch05-quench-corridor-team-a', enemyIds: ['cinder-troll-06', 'goblin-torchling-07'], threatBudget: 15, compatibilityTags: ['frontline', 'specialist', 'ranged', 'fire'], counterplay: 'Leave the marked quench lane before the troll commits, then stop the torchling’s flask from reaching the valve.', reward: { xp: 45, gold: 18, itemChoices: [] } },
  { id: 'enc-ch05-quench-corridor-team-b', enemyIds: ['iron-deserter-08', 'vault-gargoyle-08'], threatBudget: 16, compatibilityTags: ['frontline', 'ranged', 'construct'], counterplay: 'Keep the deserter from pinning you beneath the perch and stagger the gargoyle when it spreads its wings.', reward: { xp: 48, gold: 22, itemChoices: [] } },
  { id: 'enc-ch05-cinder-troll-elite', enemyIds: ['cinder-troll-08'], threatBudget: 9, compatibilityTags: ['elite', 'specialist', 'fire'], counterplay: 'Guard its announced molten charge, then attack the cracked flank while the troll catches its breath.', kind: 'lieutenant', reward: { xp: 58, gold: 26, itemChoices: [] } },
  { id: 'enc-ch05-ledger-gallery-watch-a', enemyIds: ['black-banner-08', 'ash-magus-08'], threatBudget: 17, compatibilityTags: ['mobile', 'support', 'fire', 'false-flag'], counterplay: 'Stop the magus’s focus drain before the reaver reaches the records case, then counter the exposed hex.', reward: { xp: 50, gold: 25, itemChoices: [] } },
  { id: 'enc-ch05-ledger-gallery-watch-b', enemyIds: ['iron-deserter-08', 'goblin-torchling-08'], threatBudget: 16, compatibilityTags: ['frontline', 'ranged', 'fire', 'false-flag'], counterplay: 'Use a pillar to break the torchling’s firing lane, then guard-break the veteran before it traps the case.', reward: { xp: 52, gold: 23, itemChoices: [] } },
  { id: 'enc-ch05-worker-cage-patrol-a', enemyIds: ['gloam-warg-08', 'black-banner-09'], threatBudget: 17, compatibilityTags: ['beast', 'mobile', 'false-flag'], counterplay: 'Hold the cage gate so the warg cannot flank the witness, then mark the reaver during its recovery.', reward: { xp: 55, gold: 20, itemChoices: [] } },
  { id: 'enc-ch05-worker-cage-patrol-b', enemyIds: ['vault-gargoyle-08', 'black-banner-08'], threatBudget: 16, compatibilityTags: ['construct', 'ranged', 'mobile', 'false-flag'], counterplay: 'Move out of the gargoyle’s marked firing lane and keep the reaver away from the open worker cages.', reward: { xp: 49, gold: 28, itemChoices: [] } },
  { id: 'enc-ch05-vault-gargoyle-elite', enemyIds: ['vault-gargoyle-09'], threatBudget: 10, compatibilityTags: ['elite', 'construct', 'ranged'], counterplay: 'Stagger the fixed sentinel as it spreads its wings, then break its stone plate before it regains height.', kind: 'lieutenant', reward: { xp: 62, gold: 24, itemChoices: [] } },
  { id: 'enc-ch05-cinder-heart-regulator', enemyIds: ['boss-embervault-cinder-heart-regulator', 'black-banner-09'], threatBudget: 23, compatibilityTags: ['unique', 'construct', 'specialist', 'mobile', 'frontline'], counterplay: 'Read the regulator’s marked valve lane, interrupt the relay during its pressure reset, and keep the reaver off the control rail.', kind: 'boss', bossEnemyId: 'boss-embervault-cinder-heart-regulator', reward: { xp: 92, gold: 42, itemChoices: [] } },
  ...
  { id: 'enc-ch06-ossuary-bell-jailers', enemyIds: ['bell-apostle-07', 'barrow-soldier-07'], threatBudget: 14, compatibilityTags: ['hard-control', 'leader', 'undead'], counterplay: 'Guard the raised warning bell, break the soldier’s drill call, and use the cracked pillar before the jailers close the stair.' },
  { id: 'enc-ch06-bell-loft-sentinel', enemyIds: ['vault-gargoyle-07', 'black-banner-08'], threatBudget: 17, compatibilityTags: ['construct', 'ranged', 'mobile', 'false-flag'], counterplay: 'Leave the marked bell lane before the stone sentinel drops, then stop the reaver from sounding the second alarm.', kind: 'lieutenant' },
  { id: 'enc-ch07-aqueduct-crossbow-patrol', enemyIds: ['vault-gargoyle-09', 'iron-deserter-09'], threatBudget: 19, compatibilityTags: ['construct', 'ranged', 'frontline'], counterplay: 'Take cover below the high firing slit, stagger the gargoyle as it opens its wings, and then break the shield on the bridge.', reward: { xp: 76, gold: 28, itemChoices: ['consumable-field-bandage'] } },
  { id: 'enc-ch07-compact-archive-wardens', enemyIds: ['barrow-soldier-09', 'iron-deserter-09'], threatBudget: 20, compatibilityTags: ['undead', 'leader', 'frontline'], counterplay: 'Interrupt the roster call, separate the shield veteran from the record shelf, and seize the stair before the next rotation.', kind: 'lieutenant' },
  { id: 'enc-ch08-engine-gallery-watch', enemyIds: ['black-banner-09', 'abbey-golem-09'], threatBudget: 20, compatibilityTags: ['mobile', 'construct', 'frontline'], counterplay: 'Use the pulley housing to break the reaver’s lane, then expose the golem’s numbered service plate before it seals the stair.' },
  { id: 'enc-ch08-regulator-control-crew', enemyIds: ['ash-magus-10', 'bell-apostle-09', 'black-banner-10'], threatBudget: 23, compatibilityTags: ['support', 'hard-control', 'mobile', 'fire'], counterplay: 'Jam the pressure lever during the magus’s vent, silence the bell signal, and mark the runner before it reaches the platform flags.', kind: 'lieutenant', reward: { xp: 112, gold: 34, itemChoices: ['consumable-field-bandage'] } },
];
