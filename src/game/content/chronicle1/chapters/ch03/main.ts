...
export const CH03_MAIN = Object.freeze([
  ...
  defineScene({
    ...
    choices: [
      { id: 'ch03-choice-capture-two-banner-leader', label: 'Cut off the levee leader', detail: 'Risk the exposed bank to secure a living witness and the reversible tabard.', effects: [{ type: 'flag', operation: 'add', flagId: 'two-banner-leader-captured' }, { type: 'evidence', operation: 'add', evidenceId: 'redwater-testimony' }, { type: 'combat', encounterId: 'enc-ch03-two-banner-rearguard' }], outcome: 'The leader survives and admits that both squads received pay from the same masked quartermaster.' },
      { id: 'ch03-choice-protect-evidence-ferry', label: 'Hold the evidence ferry', detail: 'Keep the proof safe while allowing more attackers to escape toward Redwater.', effects: [{ type: 'flag', operation: 'add', flagId: 'paired-cache-kit-secured' }, { type: 'combat', encounterId: 'enc-ch03-two-banner-rearguard' }, { type: 'threat', amount: 1 }], outcome: 'The ferry clears the killing ground. Riders carrying both stolen colors race ahead of you.' },
    ],
  }),
  defineScene({
