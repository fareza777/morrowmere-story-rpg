...
export const CH01_JOURNEY = Object.freeze([
  ...
  defineScene({
    id: 'ch01-living-below-toll-desk-entry', chapterId: 'ch01', region: 'gloamwood', slot: 37,
    type: 'journey', journeySubtype: 'dungeon', family: 'tollhouse-cellar-delve', weight: 8, pacing: 'danger',
    illustrationId: 'scene-ch01-living-below-toll-desk-entry', title: 'Below the Toll Desk',
    narrative: [
      'The stair drops into ankle-deep water beneath the toll desk. Six Greywatch cloaks hang from pegs with their unit badges cut away; a chain from the desk is tied to a tripwire beside the low orchard culvert.',
      'Most ration crates are empty, but one oilcloth bundle remains beneath the broken lid. Footprints turn from it toward the culvert, then stop at the first bend.',
    ],
    eligibility: { routes: ['kings-road', 'ruined-pass'], minLevel: 1, maxLevel: 2, requiredFlags: ['tollhouse-dungeon-route-chosen'] },
    requirements: [], exclusions: [], cooldownRuns: 4, oneShot: true, followUps: [], callbackPromises: [],
    dialogue: [{
      speakerName: 'Jory Fen',
      text: 'We can take the badge proof and meet whoever is guarding that drain, or collapse their approach and leave by the stair.',
      environmentIllustrationId: 'scene-ch01-living-below-toll-desk-entry',
    }],
    choices: [
      {
        id: 'ch01-choice-search-the-cellar-uniforms',
        label: 'Search the uniform crates',
        detail: 'Take the badge proof and sealed field broth, reset the toll-chain tripwire, then follow the fresh boot trail.',
        effects: [
          { type: 'flag', operation: 'add', flagId: 'stolen-greywatch-cloaks-found' },
          { type: 'flag', operation: 'add', flagId: 'tollhouse-cellar-tripwire-set' },
          { type: 'evidence', operation: 'add', evidenceId: 'removed-greywatch-badges' },
          { type: 'vitals', health: 6, resource: 2 },
          { type: 'item', operation: 'grant', itemId: 'consumable-field-bandage', quantity: 1, destination: 'pack' },
        ],
        outcome: 'The oilcloth bundle holds a usable broth and one clean bandage. You reset the toll-chain wire behind you; beneath the cut badges, a fresh boot scrape runs toward the culvert.',
        continueLabel: 'Follow the boot scrapes',
      },
      {
        id: 'ch01-choice-seal-the-cellar-culvert',
        label: 'Collapse the orchard tunnel',
        detail: 'Seal the raiders’ hidden approach and avoid a fight, leaving the stolen uniform trail unconfirmed.',
        effects: [
          { type: 'flag', operation: 'add', flagId: 'tollhouse-tunnel-collapsed' },
          { type: 'threat', amount: -1 },
        ],
        outcome: 'The weak tunnel roof comes down behind a wall of chalk. You leave by the stair before anyone can reach the trapped bell wire.',
        continueLabel: 'Leave by the tollhouse stair',
      },
    ],
  }),
  defineScene({
