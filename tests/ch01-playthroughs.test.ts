...
describe('Chapter 1 living encounter seams', () => {
  it('joins departure through Mara, the warning tree, and the tollhouse search fork', () => {
    ...
    expect(next('ch01-living-birch-marks-aftermath', 'ch01-choice-approach-tollhouse-under-cover')).toBe('ch01-living-warning-tree');
  ...
  it('joins the bypass route and queues the mandatory orchard aftermath before the war camp', () => {
    ...
    expect(next('ch01-living-split-fletched-arrow-aftermath', 'ch01-choice-preserve-the-split-fletched-arrow')).toBe('ch01-living-war-camps-last-fire');
  });
});
